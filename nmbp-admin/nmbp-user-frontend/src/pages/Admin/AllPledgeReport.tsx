...
const AllPledgeReport: React.FC = () => {
  ...
  const columns: Column<PledgeRow>[] = [
    ...
    { key: "district", label: "District" },
    {
      ...
      render: (item) => (
        <span style={{ fontWeight: 600, color: "#1A56DB" }}>
          {Number(item.totalPledges).toLocaleString()}
        </span>
      ),
    },
    {
      key: "male",
      label: "Male",
      render: (item) => Number(item.male).toLocaleString(),
    },
    {
      key: "female",
      label: "Female",
      render: (item) => Number(item.female).toLocaleString(),
    },
    {
      key: "other",
      label: "Other",
      render: (item) => Number(item.other).toLocaleString(),
    },
    { key: "dateRange", label: "Date Range" },
    {
      ...
      render: (item) => (
        <Badge
          color={item.status === "Verified" ? "green" : "orange"}
          variant="light"
          size="sm"
        >
          {item.status}
