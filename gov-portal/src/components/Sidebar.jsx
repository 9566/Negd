...
const Sidebar = ({ isOpen, onClose }) => {
  ...
  // Remove unwanted menu items and sections
  const filteredMenu = allMenuItems.filter((item) => {
    // Remove by label
    if (["Home", "NMBA", "NOS", "Label"].includes(item.label)) return false;
    // Remove by section
    if (["Home", "NMBA", "NOS", "Label"].includes(item.section)) return false;
  ...
  return (
    <>
      ...
      <nav className={`${styles.sidebar} ${isOpen ? styles.sidebarOpen : ""}`}>
        <div className={styles.roleTag}>{role.toUpperCase()} ACCESS</div>
